...
static uint8_t AC3_ParseDialnorm(const uint8_t* data, uint8_t acmod)
{
  ...
  uint32_t bits = ((uint32_t)data[6] << 16) | ((uint32_t)data[7] << 8) | data[8];
...
static void AC3_SetDialnorm(uint8_t* data, uint8_t acmod, uint8_t value)
{
  ...
  bits = (bits & ~(0x1FU << shift)) | ((uint32_t)(value & 0x1F) << shift);
  data[6] = (bits >> 16) & 0xFF;
  data[7] = (bits >> 8) & 0xFF;
  data[8] = bits & 0xFF;
}
  ...
  while (offset + 8 <= size)
  {
    ...
    if (bsid <= 10)
    {
      ...
      uint16_t crc1 = ac3_bswap16(
          (uint16_t)av_crc(crc_table, 0, frame + 4, frame_size_58 - 4));
      unsigned int crc_inv =
          AC3_pow_poly((AC3_CRC16_POLY >> 1), (8 * frame_size_58) - 16, AC3_CRC16_POLY);
      crc1 = (uint16_t)AC3_mul_poly(crc_inv, crc1, AC3_CRC16_POLY);
      frame[2] = (crc1 >> 8) & 0xFF;
      frame[3] = crc1 & 0xFF;

      // Recompute CRC-2 (covers second segment; first segment CRC is 0 so we start fresh)
      uint16_t crc2 = ac3_bswap16(
          (uint16_t)av_crc(crc_table, 0, frame + frame_size_58,
                           frame_bytes - frame_size_58 - 2));
      if (crc2 == 0x0B77)
      {
    ...
    else if (bsid <= 16)
    {
      ...
      uint16_t crc2 = ac3_bswap16(
          (uint16_t)av_crc(crc_table, 0, frame + 2, frame_bytes - 4));
      if (crc2 == 0x0B77)
      {
        frame[frame_bytes - 3] ^= 0x1;
        crc2 ^= 0x8005;
                                 ...
                                 bool wantEAC3dependent)
{
  ...
  if (bsid <= 10)
  {
    ...
    // 5-bit raw value 0-31 maps to -31 to 0 dB (dB = raw - 31)
    {
      ...
      m_info.m_dialNorm = (int)dialNormRaw - 31;
    }
